...
for model in "${models[@]}"
    ...
    do
        ...
        do
            ...
            do
                CUDA_DEVICE_ORDER=PCI_BUS_ID python ./torch_timeseries/norm_experiments/$model.py --dataset_type="$dataset" --norm_type="$norm" --norm_config=$norm_config --device="$device" --batch_size=32 --horizon=1 --pred_len="$pred_len" --windows=$windows --epochs=50 config_wandb --project="VMD_Dlinear_xiugai_visual" --name="Norm"  runs --seeds='[1]'
